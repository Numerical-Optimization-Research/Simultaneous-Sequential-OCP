...
gradient = [-10, 2, 0, 0; 0, 0, 0, 4];
hessian = [2, 0, 0; 0, 0, 12];
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = gradientDescent(aNext,gradient,setting,lambdaFix);
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = hessianFunc(aNext,gradient,setting,0,hessian);
    outputX1(n,i+1) = aNext(1);
    outputX2(n,i+1) = aNext(2);
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = gaussNewtonFunc(aNext,setting,gradient,0,0);
    outputX1(n,i+1) = aNext(1);
    outputX2(n,i+1) = aNext(2);
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = gaussNewtonFunc(aNext,setting,gradient,lambdaLM,hessian);
    outputX1(n,i+1) = aNext(1);
    outputX2(n,i+1) = aNext(2);
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = hessianFunc(aNext,gradient,setting,constHess,0);
    outputX1(n,i+1) = aNext(1);
    outputX2(n,i+1) = aNext(2);
...
for i = 1:maxIteration
    ...
    error(:,i) = soln(:) - aNext(:);
    aNext = BFGS(aCurr,aPrev,gradient,B_i);
    outputX1(n,i+1) = aNext(1);
    outputX2(n,i+1) = aNext(2);
...
[X1,X2] = meshgrid(-40:0.5:50,-10:0.5:10);
...
legend('Function Contour','1. Gradient Descent','2. Gradient Descent- Line Search','4. True Hessian','5. Gauss Newton','6. Levenberg-Marquardt','7. Constant Hessian','8. BFGS');
